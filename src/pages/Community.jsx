    ...
    mutationFn: async ({ post, currentUser }) => {
      ...
      if (!isLiked && post.author_email && post.author_email !== currentUser.email) {
        base44.functions.invoke("notifyCommunityActivity", {
          ...
          fromName: (() => { const c = customers.find(x => x.created_by === currentUser.email || x.user_email === currentUser.email); return c?.is_bean_official ? (c.display_name || "Bean") : (currentUser.display_name || currentUser.full_name || currentUser.email.split("@")[0]); })(),
